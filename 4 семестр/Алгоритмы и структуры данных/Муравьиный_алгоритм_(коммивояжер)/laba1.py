...
class Ant:
    ...
    def __init__(self, graph: Graph, alpha=2, beta=2, k=10, evaporation_rate=0.1):
        self.graph = graph
        self.alpha = alpha
        self.beta = beta
        self.k = k
    ...
    def walk(self):
        ...
        while len(self.path_nodes) != len(self.graph.nodes):  # Итерируемся сначала по вершинам
            # Вычисляем вероятность прохождения по тому или иному пути
            chances = {}
            for conn in source_node.connections:  # Затем по вершинам,
            ...
            if sum(chances.values()) == 0:
                conn = get_connection(self.graph, source_node, self.graph.nodes[0])
                conn.pheromones += round(self.k / conn.weight)
                break
